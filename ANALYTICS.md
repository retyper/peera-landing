...
## ① GA4 측정 ID 발급 → 한 줄 교체 (5분)

1. <https://analytics.google.com> → 관리(톱니) → **계정 만들기** → 속성 이름 `Peera Landing`
   → 시간대 **대한민국**, 통화 원(₩).
2. 데이터 스트림 → **웹** → URL `https://retyper.github.io/peera-landing/`, 이름 `Peera 랜딩`.
3. 발급된 **측정 ID**(`G-`로 시작)를 복사.
4. `index.html`에서 이 줄 하나만 교체:

   ```js
   window.GA_ID = '';  // ← 여기에 G-XXXXXXXXXX
   ```

   → `window.GA_ID = 'G-ABC1234567';`

비워 두면 gtag.js를 아예 안 받는다. 즉 **지금 배포해도 안전하고**, ID만 나중에 채워도 된다.

### 발급 후 GA4에서 꼭 해둘 설정

| 위치 | 설정 | 이유 |
|------|------|------|
| 관리 → 데이터 수집 → 데이터 보관 | **14개월** | 기본 2개월이라 두면 과거 데이터가 날아간다 |
